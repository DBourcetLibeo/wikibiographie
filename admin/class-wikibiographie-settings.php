...
class WikiBiographie_Settings_Page {
    ...
    public function checkbox_callback($field) {
        ...
        if( ! empty ( $field['options'] ) && is_array( $field['options'] ) ) {
            ...
            foreach( $field['options'] as $key => $label ) {
                $choices.= printf('<input type="checkbox" name="wikibiographie_options[%1$s]" value="true" %2$s><label for="%1$s">%3$s</label><br>',
                    $key,
    ...
    public function dropdown_callback($field) {
        ...
        if( ! empty ( $field['options'] ) && is_array( $field['options'] ) ) {
            ...
            printf( '<select name="wikibiographie_options[%1$s]" id="%1$s" %2$s>%3$s</select>',
                $field['id'],
                $attr,
                $choices
            );
